...
const quote = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" className="w-7 h-7 -scale-x-100 fill-blue/50" viewBox="0 0 16 16">
...
const testimonial = [
  {
    ...
    quote: "Nunca vi meu filho tão empolgado para ir à escola nas férias! Ele ama tudo!",
    rate: 4.5,
  },
  {
    name: "Carlos",
    role: "Pai da Manu (6 anos)",
    quote: "As atividades foram incríveis e organizadas. A gente se sente seguro e acolhido.",
    rate: 3.5,
  },
  {
    name: "Márcia",
    role: "Mãe da Laura (7 anos)",
    quote: "O programa de férias é completo e as crianças se divertem de verdade!",
    rate: 2,
  },
]
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      {testimonial.map((item, index) => (
        <div key={index} className="flex flex-col p-7 shadow-soft rounded-xl border-t-6 border-blue">
          <div className="flex items-center justify-between mb-4">
            {quote()}
            <Stars rating={item.rate} />
          </div>
          <p className="text-gray-600 italic mb-4">“{item.quote}”</p>
          <p className="text-blue font-semibold">- {item.name}, {item.role}</p>
        </div>
      ))}
    </div>
  );
