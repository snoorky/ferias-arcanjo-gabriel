...
const arrow = (active: boolean) => (
  <div className={`rounded-full p-2 ${active == true ? "bg-blue/20" : "bg-black/20"}`}>
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" className={`stroke-black/50 w-3 h-3 ${active == true ? "-scale-y-100" : ""}`} viewBox="0 0 16 16">
  ...
  return (
    <div className="flex flex-col gap-4">
      {faqData.map((item, index) => {
        ...
        return (
          <div key={index} className="border border-black/10 bg-white rounded-xl">
            <button className="w-full flex justify-between items-center p-4" onClick={() => toggleQuestion(index)}>
            ...
            {isOpen && (
              <>
                ...
                <p className="p-4 text-black/80" dangerouslySetInnerHTML={{ __html: item.answer }} />
              </>
            )}
          </div>
        )
